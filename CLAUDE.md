...
- **Single-pass pipeline**: Collect (all APIs concurrently) → Evaluate (decision tree) → Output
- **Optional deep analysis**: Claude AI bio/image/synthesis (toggled with `--deep`)
...
## Scoring System (v0.7 — simplified scoring architecture)
Higher score = more legitimate. Derived from evidence verdict + confidence + flag balance.

| Score Range | Label | Verdict |
|-------------|-------|---------|
| 82-100 | Verified Artist | Strong green flags, multi-platform, large fanbase |
| 58-81 | Likely Authentic | More green than red, some platform presence |
| 38-57 | Inconclusive | Mixed or insufficient evidence |
| 18-37 | Suspicious | More red than green flags |
| 0-17 | Likely Artificial | PFC label match, content farm patterns, no presence |
...
- **Tier simplification**: `--deep` flag replaces `--tier quick|standard|deep`
- **Performance**: ThreadPoolExecutor parallelization, HTTP connection pooling, in-memory cache, batched DB transactions
...
- Run: `spotify-audit <playlist-url>` (add `--deep` for Claude AI analysis)
